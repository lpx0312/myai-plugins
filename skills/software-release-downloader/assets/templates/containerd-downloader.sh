...
SCRIPT_NAME="containerd-downloader"
...
DOWNLOAD_DIR="./containerd_binaries"
DRY_RUN=false
VERIFY_MODE=false
PROXY=""
GITHUB_TOKEN=""
...
从 GitHub 下载 containerd 的所有稳定版本二进制文件 (amd64 和 arm64)

选项:
  -h, --help         显示此帮助信息并退出
  -v, --version      显示脚本版本并退出
  -d, --dir DIR      指定下载目录 (默认: ./containerd_binaries)
...
示例:
  $SCRIPT_NAME                              # 下载到默认目录
  $SCRIPT_NAME -d /opt/containerd           # 下载到指定目录
...
# 过滤稳定版本（排除 rc, beta, alpha 等预发布版本，以及 api/v* 版本）
filter_stable_versions() {
    local versions=("$@")

    for version in "${versions[@]}"; do
        # 排除包含 rc, beta, alpha 的版本
        # 排除 api/v* 格式的 API 发布版本
...
get_download_url() {
    local version="$1"
    local arch="$2"
    local filename="containerd-${version#v}-linux-${arch}.tar.gz"
