...
从 GitHub 下载 runc 的所有稳定版本二进制文件 (amd64 和 arm64)

选项:
  -h, --help         显示此帮助信息并退出
  -v, --version      显示脚本版本并退出
  -d, --dir DIR      指定下载目录 (默认: ./runc_binaries)
...
示例:
  $SCRIPT_NAME                              # 下载到默认目录
  $SCRIPT_NAME -d /opt/runc                 # 下载到指定目录
...
verify_binary() {
    ...
    # 检查文件大小（runc 二进制文件通常大于 1MB）
    local file_size
    file_size=$(stat -f%z "$file_path" 2>/dev/null || stat -c%s "$file_path" 2>/dev/null)

    if [ "$file_size" -lt 1000000 ]; then
        log_error "文件太小，可能已损坏: $(basename "$file_path") (大小: $file_size bytes)"
        rm -f "$file_path"
    ...
    if command -v file &> /dev/null; then
        # 使用 file 命令检查是否为 ELF 可执行文件
        if file "$file_path" | grep -q "ELF"; then
            if [ "$VERIFY_MODE" = true ]; then
                log_success "验证通过: $(basename "$file_path") (ELF 可执行文件)"
            fi
            return 0
        else
            log_error "文件不是有效的 ELF 可执行文件: $(basename "$file_path")"
            rm -f "$file_path"
            log_warning "已删除损坏的文件"
            return 1
        fi
    else
        # file 命令不可用时，只检查文件大小和可执行权限
        if [ -x "$file_path" ]; then
            if [ "$VERIFY_MODE" = true ]; then
                log_success "验证通过: $(basename "$file_path") (文件大小: $file_size bytes, 可执行)"
            fi
            return 0
        else
            # 文件不可执行，但可能是刚下载还没设置权限的情况
            # 尝试设置可执行权限后再次验证
            chmod +x "$file_path" 2>/dev/null
            if [ "$VERIFY_MODE" = true ]; then
                log_success "验证通过: $(basename "$file_path") (文件大小: $file_size bytes)"
            fi
            return 0
        fi
    fi
...
download_file() {
    ...
    if [ -f "$output_path" ]; then
        ...
        if verify_binary "$output_path"; then
            return 0
        else
